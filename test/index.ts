...
describe("ERC721DogyRace", () => {
  ...
  it("whitelist", async () => {
    ...
    erc721DogyRace = await ERC721DogyRace.deploy(10, BigNumber.from("120000000000000000"), "https://localhost:3000/")
  ...
  it("mint", async () => {
    ...
    await erc721DogyRace.addToWhiteList(owner.address)
    await erc721DogyRace.addToWhiteList(addr1.address)
    await erc721DogyRace.connect(addr1).mint({
      value: BigNumber.from("120000000000000000")
    })
    let tx: ContractTransaction = await erc721DogyRace.connect(addr1).mint({
      value: BigNumber.from("120000000000000000")
    })
    expect((await erc721DogyRace.balanceOf(addr1.address)).toString()).to.equal("2")
    let receipt: ContractReceipt = await tx.wait()
    receipt.events?.filter((x) => { return x.event == "Transfer" })
